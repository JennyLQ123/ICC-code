...
class PPO(object):
    ...
    def __init__(self):
        ...
        with tf.variable_scope('critic'):
            l1 = tf.layers.dense(self.tfs, 110, tf.nn.relu)
            l2 = tf.layers.dense(l1, 150, tf.nn.relu)
    ...
    def _build_anet(self, name, trainable):
        with tf.variable_scope(name):
            l1 = tf.layers.dense(self.tfs, 110, tf.nn.leaky_relu, trainable=trainable)
            l2 = tf.layers.dense(l1, 150, tf.nn.leaky_relu, trainable=trainable)
            l3 = tf.layers.dense(l2, 150, tf.nn.leaky_relu, trainable=trainable)
            ...
            norm_dist = tf.distributions.Normal(loc=mu, scale=sigma)  #zhengtai fenbu
