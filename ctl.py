...
routeid_member_handle={} #eg. routeid_member_handle={"010819":8,"020819":9}
...
def writeFwdTable(switch_list,flow_rules,route_id,dst_ip_addr,dst_eth_addr):
    ...
    flowsHandleIDdic[route_id][dNode]=[last_hop,str(SWITCH_TO_HOST_PORT)]
...
def flowRulesModify(switch_list,src_dst_pair,flow_rules,dst_eth_addr):
    #delete old path
...
def reset(data_info,flag,switch_list):
    for p_ in UPATH:
        flowRulesModify(switch_list,p_,PATHS[p_][0],dst_eth_addr=S2ETH[p_[-2:]])
...
def controllerMain(data_info,flag):
    ...
    ppo = PPO()
    all_ep_r=[]
    #ppo.load("model_flow/ppo%d"%(EP_START,))
    #with open("res_load_v2","r") as f:
    ...
    for ep in range(EP_START,EP_MAX):
        ...
        if (ep%10==0):
            ppo.save("model_flow/ppo%d"%(ep,))
        with open("flow_load","w") as f:
