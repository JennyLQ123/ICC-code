...
def controllerMain(data_info,flag):
    ...
    for p_ in UPATH:
        old_routeid_path[p_]={}
        flowRulesInit(switch_list,routeid_list=["01"+p_,"02"+p_],flowRules_list=[PATHS[p_][0],PATHS[p_][0]],dst_ip_addr=S2IP[p_[-2:]], dst_eth_addr=S2ETH[p_[-2:]])
        old_routeid_path[p_]["01"+p_]=PATHS[p_][0]
        old_routeid_path[p_]["02"+p_]=PATHS[p_][0]
    ...
    for ep in range(EP_START,EP_MAX):
        ...
        with open("reward/sp_1.2","w") as f:
